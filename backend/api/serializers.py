...
class RecipePostSerializer(ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not tags:
            raise ValidationError(
                {'tags': 'Вы не добавили теги в рецепт.'}
            )
...
class RecipeMinifiedSerializer(ModelSerializer):
    """Сериализатор списка рецептов для работы со списком покупок."""
...
class GetRemoveSubscriptionSerializer(ModelSerializer):
    """Сериализатор добавления и удаления подписок пользователей."""
